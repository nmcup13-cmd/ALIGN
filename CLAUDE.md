...
### Deploy: Vercel + Firebase split (as of 2026-09-11)

Live at **https://align-app-sage.vercel.app**, deployed on **Vercel**, not Firebase App Hosting — `align-tech-stack.md` §2.2 recommends App Hosting, but that requires upgrading the `nmc-align-2026` Firebase project to the Blaze plan first (blocked/deferred, same reason as [[docs/05-log/2026-09-06-firebase-storage-blaze-deferred-log|Storage]]). See [[docs/05-log/2026-09-11-deploy-to-vercel-log|05-log/2026-09-11-deploy-to-vercel-log]] for the three deploy-blocking issues hit and fixed (jose/jwks-rsa ESM crash, Vercel's git-commit-author block, missing Root Directory setting). `align-app/apphosting.yaml`/`firebase.json` are kept but currently unused — only relevant again if this later moves back to Firebase App Hosting.

Splitting hosting (Vercel) from backend (Firebase Auth/Firestore/Storage) is a normal, fully-supported pattern — Firebase's client/Admin SDKs work the same regardless of where the app is hosted — but it does mean:
- **`firestore.rules` does not deploy via `git push`.** Vercel's GitHub integration only builds/deploys `align-app/`; a `firestore.rules` change needs its own `firebase deploy --only firestore:rules` (run from `align-app/`, where `firebase.json`/`.firebaserc` live) — don't assume a rules edit is live just because the app redeployed.
- **Env vars are duplicated in two places**: `align-app/.env.local` (local dev) and the Vercel project's Environment Variables (Production + Preview) — see `align-app/package.json`'s sibling `.env.example` for the full list. Rotating the Firebase service account key or any `NEXT_PUBLIC_*` value means updating both, not just one.
- **Deploying**: prefer `git push origin main` (Vercel's GitHub integration builds it in the cloud) over running `vercel deploy --prod` from a possibly-flaky local connection. If a deploy seems to hang as "Building…" or shows status `UNKNOWN` in the `vercel` CLI, don't trust that output — query `https://api.vercel.com/v13/deployments/{id}` directly (needs a token from `~/.local or %APPDATA%/com.vercel.cli`) and check `readyState`/`readyStateReason`; the CLI has been observed to hide a real `BLOCKED` or `ERROR` state behind a misleading "still building" display.
