...
- เพิ่ม CLO ใหม่, ผูก/ถอด PLO ผ่าน chip, **ลบ CLO** (ผ่าน confirm dialog ก่อนเสมอ), เพิ่ม/แก้ไข/ลบหัวข้อ course syllabus ในหน้า "จัดการรายวิชา"
...
- จำลองการสร้างเอกสาร Word (สถานะ idle → generating → done) — Word Export Panel แสดง **Area of Improvement เสมอ ไม่มี toggle ให้เลือกซ่อน** (เดิมมี toggle รวม/ไม่รวมตาม AB-18 แต่ผู้ใช้ยืนยันตัดฟีเจอร์นี้ออกจาก scope แล้ว 2026-08-20 — ดู [[../../00-archived/backlog-ab-18-area-of-improvement-toggle|00-archived/backlog-ab-18-area-of-improvement-toggle]] — ปรับ `WordExportPanel.dc.html`/`M-WordExportPanel.dc.html` แล้ว republish canvas เรียบร้อย)
...
- **[sync แล้ว — 2026-08-23]** ไฟล์ต้นฉบับ sync กับฟีเจอร์ลบ CLO ใน source of truth แล้ว ([[align-app-screens#หน้าจอ 3 — จัดการรายวิชา: CLO และผูก PLO|align-app-screens หน้าจอ 3]]): เพิ่มปุ่มลบต่อการ์ด CLO (ไอคอนถังขยะ, ghost, hover เป็นสี `status.gap`) + Confirm Dialog (หัวข้อ, รหัส/คำอธิบาย CLO, ข้อความอธิบายผลกระทบ soft-delete, ปุ่ม "ยกเลิก"/"ลบ CLO นี้") ให้ทั้ง `CourseManage.dc.html`, `M-CourseManage.dc.html`, และ `align-connected-mobile-prototype.html` (ทดสอบ flow เปิด/ยกเลิก/ยืนยันลบจริงผ่าน local server แล้ว ลบสำเร็จและอัปเดตจำนวน CLO ถูกต้อง) — **หมายเหตุ**: นี่คือการแก้ไฟล์ต้นฉบับ (`.dc.html`/HTML) ในโฟลเดอร์นี้เท่านั้น ลิงก์ Artifact ที่เผยแพร่ไว้ที่ด้านบนของเอกสารนี้ยังเป็นเวอร์ชันเก่า (seed ล่าสุดก่อนหน้านี้) ยังไม่ได้ republish รอบใหม่ — republish เมื่อสะดวก/เมื่อมีการเปลี่ยนแปลงอื่นสะสมเพิ่มเติม เพื่อลดจำนวนรอบ publish
