...
- **[ตกรุ่น — 2026-08-23]** ต้นแบบที่คลิกได้**ตกรุ่นแล้ว**สำหรับฟีเจอร์ลบ CLO ที่เพิ่งเพิ่มเข้าไปในเอกสาร source of truth ([[align-app-screens#หน้าจอ 3 — จัดการรายวิชา: CLO และผูก PLO|align-app-screens หน้าจอ 3]]): ทั้ง `CourseManage.dc.html`/`M-CourseManage.dc.html` (Design Canvas ในโฟลเดอร์นี้) และ `align-connected-mobile-prototype.html` มีเฉพาะปุ่ม "ลบ" ของหัวข้อ syllabus เท่านั้น **ยังไม่มีปุ่มลบต่อการ์ด CLO และยังไม่มี Confirm Dialog** ตามที่ระบุไว้ใหม่ — ต้องอัปเดตผ่าน `design` skill (เอกสารนี้ไม่มีเครื่องมือแก้ไข `.dc.html`/HTML เหล่านี้เอง)
