...
| **Auditability** | `evidence_access_log` ต้อง append-only — แก้ไข/ลบไม่ได้แม้โดยบัญชี `program_admin` ผ่านช่องทางปกติของระบบ | AB-07, กฎ #5 | ตรวจว่าไม่มี endpoint ใดใน [[align-technical-design|align-technical-design]] §3 ที่ update/delete collection `evidence_access_log` |
