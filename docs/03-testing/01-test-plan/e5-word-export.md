...
| TC-AB16-04 | `[PLACEHOLDER — รอคำตอบ]` นิยาม 3 ระดับ | เอกสาร Word ของ `COS101` มีข้อมูล gap ประกอบหลายแหล่ง (ไม่มีหลักฐาน / match % ต่ำ / gap เทียบ syllabus) | สร้างส่วน Area of Improvement | **ยังไม่สามารถระบุได้ว่า "3 ระดับ" ที่ AC กำหนดคืออะไร** (ระดับ CLO/รายวิชา/หลักสูตร? หรือ ต่ำ/กลาง/สูง? หรืออื่นๆ) ไม่มีนิยามในสเปคต้นฉบับ (ดู [[test-plan-align|test-plan-align §6.3]]) — เขียน test case ตรวจโครงสร้าง 3 ระดับที่ชัดเจนต่อเมื่อได้คำตอบแล้ว ปัจจุบันทดสอบได้เฉพาะว่า "มีส่วนนี้และเนื้อหาอ้างอิงข้อมูลจริง" (TC-AB16-01) เท่านั้น | AB-16, คำถามเปิด §6.3 |
