...
**สรุป**: รวม 154 test case ครอบคลุมทั้ง 26 User Stories (AB-01–AB-27 ไม่รวม AB-18) — ดูรายละเอียดขอบเขต/ชุดข้อมูลทดสอบ/คำถามเปิดทั้งหมดใน [[test-plan-align|test-plan-align]] **ปิดคำถามเปิดครบทุกข้อแล้ว** (2026-08-20): "ความถี่ที่แมทช์" §6.1, โครงสร้างข้อมูล course syllabus + algorithm จับคู่คำพ้องความหมาย §6.2, "Area of Improvement 3 ระดับ" §6.3, AB-18 ถูกตัดออกจาก scope §6.4, ฟิลด์ฟอร์มสมัครสมาชิก §6.5, `rejection_reason` บังคับกรอกหรือไม่ §6.6, วิธีสร้างบัญชี `program_admin` ชุดแรก §6.7 (อัปเดต 2026-08-23: เพิ่ม 8 test case ใหม่ใน E1/E2/E3 ครอบคลุม soft-delete `plo`/`evidence` และ snapshot ถาวรของ `ai_match_result.linked_plo_ids` ตาม `align-api-schema-design.md` §4.1/§5.1/§5.3 ที่ยืนยันแล้ว; อัปเดต 2026-08-23 รอบที่ 2: ผู้ใช้ยืนยันว่า CLO soft-delete เป็นฟีเจอร์ที่ต้องการจริงครบ 5/5 entity ตาม §3.3/§4.1/§5.1 — เพิ่ม TC-AB02-05/06/07 ใน E1 ครอบคลุม soft-delete CLO ที่มี `ai_match_result` (รวม confirmed) อ้างอิงอยู่ และการ re-evaluate `clo_plo_ready`/`total_clo_count` ทันที; อัปเดต 2026-09-05: เพิ่ม test case รองรับ 2 entity ใหม่ที่เพิ่งเข้า schema — `notification` (§2.14/§3.15, กลไกบังคับใช้ BR#2/AB-12) เพิ่ม TC-AB12-06–10 ใน E4 ครอบคลุม auto-resolve, Firestore transaction แบบ query-then-write กันแจ้งเตือนซ้ำ (ปรับกลไก 2026-09-06 ให้ตรงกับ Firestore), reopen สร้าง document ใหม่ไม่ reuse document เดิม, และ PDPA scope; `account_approval_log` (§2.15/§3.14, audit trail ของ AB-26) เพิ่ม TC-AB26-08–10 ใน E6 ครอบคลุมการเขียน document ใหม่ผ่าน Firestore transaction เดียวกับการอัปเดต `user` (ปรับกลไก 2026-09-06 ให้ตรงกับ Firestore), append-only ไม่ลบ/เขียนทับประวัติ, และ `decided_by` ต้องเป็น `program_admin` เท่านั้น (บังคับที่ Cloud Function + Firestore Security Rules เป็น defense-in-depth) — ดูรายละเอียดในหัวข้อ 5 ของ [[test-plan-align|test-plan-align]])
