...
โครงสร้างนี้ **ยืนยันแล้ว** ตามหัวข้อ 6.2: syllabus แบ่งเป็น 2 ส่วนคู่กัน — (ก) ไฟล์ syllabus ทางการ (`origin_file_ref`) ที่อัปโหลดเป็นไฟล์แนบและระบบไม่ parse เนื้อหา และ (ข) หัวข้อที่วางแผนสอนแต่ละสัปดาห์แบบโครงสร้าง (`content` = `[{week_no, topic, detail}]`) ซึ่งเป็นข้อมูลที่ AI gap analysis (AB-22) ใช้เป็น input จริง ตัวอย่างด้านล่างคือฟิลด์ `content` ส่วน (ข):
...
- **E4**: เพิ่ม TC-AB12-06/07/08/09/10 ใน [[e4-dashboard-alerts|e4-dashboard-alerts]] ครอบคลุมกลไก `notification` (BR#2, AB-12, T-042–T-045) ครบ 5 พฤติกรรม — auto-resolve อัตโนมัติเมื่อ CLO มีผลจับคู่ `confirmed` แล้ว (TC-AB12-06), Firestore transaction แบบ query-then-write กันสร้างแจ้งเตือนซ้ำสำหรับ `clo_id` เดียวกันที่ยัง unresolved (TC-AB12-07), reopen ต้องสร้าง document ใหม่ไม่ reuse document เดิมที่ resolved ไปแล้ว (TC-AB12-08), แจ้งเตือนที่ resolved หายจากแดชบอร์ดเองทันทีโดยผู้ใช้ไม่ต้องทำอะไร (TC-AB12-09), และขอบเขตสิทธิ์ PDPA เห็นเฉพาะแจ้งเตือนของตนเอง (TC-AB12-10) — จาก 20 เป็น **25** (happy path 9→11, edge case 5→6, rejection 6→8)
- **E6**: เพิ่ม TC-AB26-08/09/10 ใน [[e6-user-registration-approval|e6-user-registration-approval]] ครอบคลุม `account_approval_log` (audit trail ของ AB-26) ครบ 3 พฤติกรรม — เขียน document ใหม่พร้อมกับอัปเดต `user.account_status`/`approved_by`/`approved_at` ผ่าน Firestore transaction (`runTransaction`) เดียวกันเสมอ (TC-AB26-08), เป็น append-only ไม่ลบ/ไม่เขียนทับประวัติเดิมแม้บัญชีเดียวกันถูกตัดสินใจซ้ำหลายรอบ (TC-AB26-09), และ `decided_by` ต้องเป็นบัญชี `role=program_admin` เท่านั้น — บังคับที่ Cloud Function ก่อนเขียน (กลไกหลัก) และ Firestore Security Rules ปฏิเสธการเขียนตรงจาก client ทุกกรณี (defense-in-depth ชั้นที่สอง — ไม่ใช่ DB-level constraint เพราะ Firestore ไม่มีกลไกเทียบเท่า) (TC-AB26-10) — จาก 25 เป็น **28** (happy path 12→14, edge case คงที่ 4, rejection 9→10)
- **รวมทั้งหมด**: เพิ่มจาก 146 เป็น **154 test case** placeholder ยังคงเป็น 0 รายการ)
- (อัปเดต 2026-09-06 — audit พบว่า TC-AB12-07/TC-AB26-08/TC-AB26-10 (E4/E6) และ TC-AB09-02 (E3) ยังบรรยายกลไกด้วยศัพท์ SQL-specific (unique constraint, DB-level constraint, commit/rollback) ที่ไม่มีจริงใน Firestore — แก้เฉพาะคำอธิบายกลไกให้ตรงกับ `align-technical-design.md` §2.0/§2.0.4 และ `align-api-schema-design.md` §2.6/§2.7/§2.8 (Firestore transaction, Cloud Function check, Security Rules defense-in-depth) ผลลัพธ์ทางธุรกิจที่ทดสอบไม่เปลี่ยนแปลง จำนวน test case ไม่เปลี่ยน — รายละเอียดที่ [[../../05-log/2026-09-05-notification-account-approval-log|05-log/2026-09-05-notification-account-approval-log §อัปเดต 2026-09-06]])
